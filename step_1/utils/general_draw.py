...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            # ax.set_ylim(0, 1.01)
            # fig.tight_layout()
            # name='pic/PR_curve_'+str(count)+'.png'
...
def ap_per_class(tp, conf, pred_cls, target_cls,names):
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_l == 0:
            ...
            # AP from recall-precision curve
            for j in range(tp.shape[1]):
                ap[ci, j], mpre, mrec = compute_ap2(recall[:, j], precision[:, j])
                if plot and j == 0:
    ...
    f1 = 2 * p * r / (p + r + 1e-16)
    if plot:
        plot_pr_curve(names,px, py, ap, Path(save_dir) / 'PR_curve.png')
...
def plot_pr_curve(names,px, py, ap, save_dir='pr_curve.png'):
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=2, label=f'{names[i]} {ap[i, 0]:.3f}')  # plot(recall, precision)
    else:
    ...
    fig.savefig('pic/PR_curve_or1.png', dpi=800)
...
def plot_mc_curve(px, py, save_dir='mc_curve.png', names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    fig.savefig('pic/PR_curve_or2.png', dpi=250)
...
def compute_ap2(recall, precision):
    """ Compute the average precision, given the recall and precision curves
    # Arguments
        recall:    The recall curve (list)
        precision: The precision curve (list)
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.5, merge=False, classes=None, agnostic=False):
...
def plot_one_box(csvname,x, img, color=None, label=None, line_thickness=None):
...
def plot_images(images,works,batchi, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    ...
    # Check if we should resize（第1213行）
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            boxes[[1, 3]] += block_y
            count=0
            img_name = os.path.basename(paths[i])[:40].split(".")[0]
            csvname="D:\Tuo\yolov5-cell\csv/"+works+"_"+img_name+'.csv'
            if os._exists(csvname):
                os.remove(csvname)
            csvfile = open(csvname, 'w', encoding='utf-8',newline="")
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(["num","x0", "y0", "x1","y1"])
            for j, box in enumerate(boxes.T):
                ...
                if gt or conf[j] > 0.3:  # 0.3 conf thresh
                    ...
                    csv_writer.writerow([str(count),str(int(box[0])), str(int(box[1])), str(int(box[2])), str(int(box[3]))])
                    plot_one_box(csvname,box, mosaic, label=label, color=color, line_thickness=tl)
                    count+=1
            f = "count.txt"
            with open(f, "a") as file:
                file.write(works+">>>"+str(count)+" ")
