...
def crop_one_box(x,count,path, img, label=None):
    path=path.split("/")[-1].split(".")[0]
    #split
    print("左上点的坐标为：(" + str(int(x[0])) + "," + str(int(x[1])) + ")，右下点的坐标为(" + str(int(x[2])) + "," + str(int(x[3])) + ")")
    obj_img = img[int(x[1]):int(x[3]), int(x[0]):int(x[2])]  # cv2裁剪出目标框中的图片
    my_file = Path("D:\Tuo\yolov5-cell\detect/Output" + '\\' +path )  # 判断当前name对应的类别有无文件夹
    if 1 - my_file.is_dir():  # 无则新建
        os.mkdir(my_file)
    cv2.imwrite(str(my_file) + '\\' + path+"_"+label.split(" ")[0]+"_"+str(count)+".jpg",obj_img)  # 保存裁剪图片，图片命名4位，不足补0
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    cv2.rectangle(img, c1, c2, color=(0,0,255), thickness=4, lineType=cv2.LINE_AA)
    if label:
