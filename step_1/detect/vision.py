...
def GetFileFromThisRootDir(dir, ext=None):
    ...
    for root, dirs, files in os.walk(dir):
        for filespath in files:
            # if "000011" not in filespath and "030" not in filespath and "055" not in filespath and "090" not in filespath:
            #     continue
...
def visualise_gt(label_path, pic_path, newpic_path, hbb=True):
    ...
    for result in results:
        ...
        for i in range(len(boxes)):
            if boxes[i][-1].lower() == "pf":
                line_color = (44,160,44)

            elif boxes[i][-1][:2].lower() == "pm":
                line_color = (180,119,31)

            elif boxes[i][-1][:2].lower() == "po":
                line_color = (14,127,255)

            elif boxes[i][-1][:2].lower() == "pv":
                line_color = (40,39,214)

            if hbb:
                cv2.rectangle(im, (boxes[i][0], boxes[i][1]), (boxes[i][2], boxes[i][3]), line_color, 10)
            else:
                box = np.array([[boxes[i][0], boxes[i][1]], [boxes[i][2], boxes[i][3]], \
                                [boxes[i][4], boxes[i][5]], [boxes[i][6], boxes[i][7]]], np.int32)
                cv2.polylines(im, [box], True, (0, 0, 0), 5)  # true表示闭合
            # 书写标签
            #cv2.rectangle(im, (boxes[i][0], boxes[i][1] - 15), (boxes[i][0] + 65, boxes[i][1] - 2), (255, 0, 0),
            #              thickness=0)  # thickness表示线的粗细，等于-1表示填充，颜色为(255, 0, 0)
            cv2.putText(im, boxes[i][-1], (boxes[i][0], boxes[i][1] - 2), font, 5, line_color,
                        20)  # 0.5是字体大小，2是字体的粗细
...
if __name__ == '__main__':
    ...
    newpic_path = os.path.join(root, 'vision')  # 可视化结果的保存路径
